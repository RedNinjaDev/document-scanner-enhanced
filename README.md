...
*Document Scanner Enhanced* is a fork of GNOME's [Document Scanner](https://gitlab.gnome.org/GNOME/simple-scan) that adds Auto-Crop (on by default) and Auto-Straighten (off by default) toggles in Preferences → Postprocessing for documents scanned against a dark scanner bed.
