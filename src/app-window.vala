...
public class AppWindow : Adw.ApplicationWindow
{
    ...
    private async string? choose_file_location ()
    {
        ...
        else
        {
            ...
            string extension = mime_type_to_extension (save_format);
            string default_name = _("Scanned Document") + "." + extension;
            if (settings.get_boolean ("autoname-enabled") && book.n_pages > 0)
            {
                var suggested = yield AutoNamer.suggest_filename (book.get_page (0),
